...
separatorDraw = {'d001': [dummyD.get('dummy50'), valve.get('fv001'), 50, 100], 'd002': [valve.get('fv001'), dummyD.get('dummy0'), 50, 300] }
transmitterDraw = {'pi001': [separator.get('d001'), 10, 10], 'li001': [separator.get('d001'), 200, 300],
                   'li003': [separator.get('d001'), 150, 10], 'fi001': [valve.get('fv001'), 400, 100], 'li002': [separator.get('d002'), 600, 200]}
controllerDraw = {}
...
def new_sep(tag='ddd', volume=8, volume_water=4, source='dummy0', out_source='fv001', x=1000, y=100):
    tag = input('Tag: ')
    x = mPos[0]
    y = mPos[1]
    out = input('Out: ')
...
def new_transmitter(tag='lll', typ='level oil', x=100, y=100, source='dummy0'):
    tag = input('Tag: ')
...
def new_valve(tag='v001', typ='oil', size=1, source='dummy0', out_source='dummy0', x=1000, y=100):
    tag = input('Tag: ')
    x = mPos[0]
    y = mPos[1]
...
'''def new_controller(tag='v001', typ='oil', size=1, source=dummy, out_source=dummy, x=1000, y=100):
    tag = input('Tag: ')
    x = mPos[0]
    y = mPos[1]
    valve.setdefault(tag, Valve(tag, typ, size))
...
def draw_controller():
    for tag in controllerDraw:
        controller.get(tag).draw(controllerDraw.get(tag)[0], controllerDraw.get(tag)[2],
                                 controllerDraw.get(tag)[3], controllerDraw.get(tag)[4],
                                 controllerDraw.get(tag)[5], controllerDraw.get(tag)[6])
...
def info_box(self, source, out_source='dummy0'):
    global tagInfo
...
#new_sep('d001', 8, 4, dummyD.get('dummy50'), valve.get('fv001'), 300, 200)
#new_sep('d002', 8, 4, valve.get('fv001'), dummyD.get('dummy0'), 700, 300)
#new_valve()
